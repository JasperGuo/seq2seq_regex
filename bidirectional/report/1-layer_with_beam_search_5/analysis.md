## 1-layer Bidirectional with Beam Search Analysis Result
...
| lines with 1 or more character , or ending with the string `<M0>` | `((.)+)|((.*)(<M0>))`                    | Correct                                  |
...
| lines containing a letter or string `<M0>` | `.*([<LET>])|(<M0>).*`                   | Correct                                  |
...
| lines containing the string `<M0>` followed by either the string `<M1>` , a letter , or a capital letter | `(<M0>).*((<M1>)|([<LET>])|([<CAP>])).*` | Correct                                  |
| lines with a capital letter after a vowel at least zero times | `([<VOW>]).*(([<CAP>])*).*`              | Correct                                  |
| lines ending with a number after a vowel | `((.*)([<VOW>])).*([<NUM>]).*`           |                                          |
| lines containing a vowel or string `<M0>` | `.*([<VOW>])|(<M0>).*`                   | Correct                                  |
...
| **lines containing a number after a vowel 5 or more times** | `([<VOW>]).*(([<NUM>]){5,}).*`           | Correct                                  |
...
| **lines that contain a number followed by a character , at least 4 times** | `([<NUM>]).*((.){4,}).*`                 | Correct                                  |
...
| lines with the string `<M0>` before string `<M1>` followed by string `<M2>` | `(<M0>).*(<M1>.*<M2>.*).*`               | Correct                                  |
...
| **lines that have 4 or more numbers**    | `.*([<NUM>]){4,}.*`                      | Correct                                  |
| **lines with a character after a letter at least 5 times** | `([<LET>]).*((.){5,}).*`                 | Correct                                  |
